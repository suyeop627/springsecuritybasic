...
public class JwtTokenVerifier extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(HttpServletRequest httpServletRequest,
    ...
    if (authorizationHeader.isBlank() || authorizationHeader == null || !authorizationHeader.startsWith(jwtConfig.getTokenPrefix())) {
      String head = jwtConfig.getAuthorizationHeader();
      filterChain.doFilter(httpServletRequest, httpServletResponse);
      return;
    }
    String token = authorizationHeader.replace(jwtConfig.getTokenPrefix(), "");
    try {
     // String secretKey = "secure_secure_secure_secure_secure_secure_secure_secure_secure_secure_";

      Jws<Claims> claimsJws = Jwts.parser()
          .setSigningKey(secretKey)
          .build()
          .parseClaimsJws(token);
    //jwt payload에 담긴 정보 key-value 형태
